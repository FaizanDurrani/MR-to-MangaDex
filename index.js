...
const { email, password } = require('./config'); // Get email and password from config file

let tachiObj = { // Placeholder Object information for Tachi
    "version": 2,
    "mangas": []
};
...
request(opt1, function (err, response) {
    ...
    request(opt2, function (err, res) {
        ...
        for (let i of userManga) {
            ...
            oidObject['lastReadChapterOID'] = '0'; // Placeholder 0 for lastReadChapterOID
            if (entry['lastReadChapterOID'] !== null) // If the current manga has a lastReadChapterOID then set the oidObject lastReadChapterOID to that
                oidObject['lastReadChapterOID'] = entry['lastReadChapterOID'];
        ...
        request(opt3, function (err, res) {
            ...
            for (let entry of mangaInfo) {
                ...
                for (let manga of mangaList) {
                    ...
                    if (manga['lastReadChapterOID']) // If the manga object contains "lastReadChapterOID", set lastRead to that
                        lastRead = manga['lastReadChapterOID'];
                    ...
                    tachiObj.mangas.push({ // Push a completed manga object to the tachiObj
                        ...
                        "chapters": [{
                            "u": `/pagesv2?oid\=${lastRead}`,
                            "r": 1
                        }],
                        "history": [[
                            `/pagesv2?oid=${lastRead}`,
                            Date.now()
                        ]]
                    })
                }
